...
Cocos Creator 的脚本主要是通过扩展组件来进行开发的。目前 Cocos Creator 支持 **JavaScript和 TypeScript ** 两种脚本语言。通过编写脚本组件，并将它赋予到场景节点中来驱动场景中的物体。
...
```ts
@property({ type: [CCString] })  // 简写 @property([CCString])
names: string[] = [];

@property({ type: [Node] }) // 简写 @property([Node])
enemies: Node[] = [];
```
...
组件脚本的【初始化阶段】，我们提供了 `onLoad` 回调函数。

`onLoad` 回调会在节点【首次激活时触发】，比如所在的场景被载入，或者所在节点被激活的情况下。

在 `onLoad` 阶段，保证了你可以【获取到场景中的其他节点，以及节点关联的资源数据】。

onLoad 总是会在任何 start 方法调用前执行，这能用于安排脚本的初始化顺序。通常我们会在 `onLoad` 阶段去做一些【初始化相关的操作】。例如：
...
【操作：脚本前面的勾选，勾选就是 enabled=true ，表示 组件启用时监听按钮点击事件】

### 1.4.3 start

【注意：与onEnable激活相关，没激活 也不执行】

`start` 回调函数会在【组件第一次激活前，也就是第一次执行 `update` 之前触发】。`start` 通常用于初始化一些中间状态的数据，这些数据可能在 update 时会发生改变，并且被频繁的 enable 和 disable。
...
游戏开发的一个关键点是在【每一帧渲染前更新物体的行为，状态和方位】。这些更新操作通常都放在 `update`回调中。

fps: 60。1秒钟画面会被渲染60次，随物理设备波动而波动

dt: 参数dt是 上一帧 跟 下一帧 相隔的时间差
...
```ts
import { _decorator, Button, Component, Vec3, find } from 'cc';
const { ccclass, property } = _decorator;

@ccclass('S04Component')
export class S04Component extends Component {
    // 在属性检查器中拖入需要移动的 Button 组件。
    @property({
        type: Button,
        displayName: '移动按钮',
        tooltip: '需要向上移动的按钮组件',
    })
    pageButton: Button | null = null;
    ...
    // 复用同一个 Vec3，避免在每一帧创建新的坐标对象。
    private readonly buttonPosition = new Vec3();

    onLoad() {
        // 如果属性检查器没有绑定按钮，则按场景路径自动查找。
        if (!this.pageButton) {
            const buttonNode = find('Canvas/Button');
            this.pageButton = buttonNode?.getComponent(Button) ?? null;
        }

        // 两种方式都没有获得按钮时，只提示一次，update 会安全退出。
        if (!this.pageButton) {
            console.warn('未找到按钮，请将 Button 组件拖到“移动按钮”属性中。');
        }
    ...
    update(deltaTime: number) {
        // update 每帧执行；按钮不存在或已被销毁时不再处理。
        if (!this.pageButton?.isValid) {
            return;
        }

        // Button 是组件，实际需要移动的是它所在的 Node。
        const buttonNode = this.pageButton.node;

        // 读取按钮相对于父节点的当前坐标。
        buttonNode.getPosition(this.buttonPosition);

        // 本帧位移 = 每秒速度 * 本帧耗时，保证不同帧率下速度一致。
        this.buttonPosition.y += this.moveSpeed * deltaTime;

        // 将修改后的坐标设置回节点；Y 增大表示向上移动。
        buttonNode.setPosition(this.buttonPosition);
    }
...
【执行时机：跟update一样也是一直执行，但是是在update执行时机之后】
...
`onDisable()` 是 Cocos Creator 的生命周期函数：当组件从“启用状态”变成“禁用状态”时自动调用。

onDisable：暂时停止，可以重新启用，可能执行多次
...
​	// 加载新场景时，旧场景中的节点和组件会被销毁，因此会触发 `onDestroy()`。使用 `director.addPersistRootNode()` 设置为常驻节点的对象除外。
...
console.log('组件所在节点：', currentNode);
console.log('节点名称：', currentNode.name);
```
...
export class S04Component extends Component {
    start() {
        ...
        // 当前节点没有 Label 组件时直接结束，避免空对象报错
        if (!label) {
            console.warn('当前节点上没有 Label 组件');
            return;
        }

        const text = this.name + ' started';
...
// 【场景名称不需要填写 .scene 后缀】
const accepted = cc.director.loadScene("MyScene");
// accepted = true：不代表场景已经加载完成，只代表加载请求已成功启动；
// accepted = false：请求没有开始，例如场景不存在，或者正在加载另一个场景
if (!accepted) {
    console.error('场景切换请求未被接受');
}
...
有些场景的【加载时间过长，会造成游戏的卡顿】，这时我们可以使用【预加载】的方式，在时间充裕的时候加载场景数据，将【这些数据缓存在内存中】，这种方式本质上是一种“空间换时间”的方式。**预加载**的操作代码如下所示。
...
/*
	loadScene：加载并切换到目标场景
	preloadScene：只提前加载，不切换场景
*/
import { director } from 'cc';

// table.scene
director.preloadScene('table', (error, sceneAsset) => {
    if (error) {
        console.error('table 场景预加载失败：', error);
        return;
    }

    console.log('下一个场景已预加载：', sceneAsset?.name);
    // 预加载完成后，仍然需要调用 loadScene() 才会真正切换场景
...
如果我们需要用一个组件控制所有场景的加载，或在场景之间传递参数数据，就需要将该组件所在节点标记为【常驻节点】，使它在场景切换时不被自动销毁，常驻内存。【常驻节点上的组件和数据也会继续保留在于新场景中】。

常用场景：背景音乐播放器；玩家全局数据；网络连接管理器；场景之间共享的管理组件。
...
// 判断是否为常驻节点
const persistent = director.isPersistRootNode(myNode);
